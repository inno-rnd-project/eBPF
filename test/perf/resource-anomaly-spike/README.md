...
- `netobs-gpuobs-correlation` PrometheusRule의 `netobs-gpuobs.resource-anomaly-spike.recording` group이 적용된 후 최소 30초 경과(base record는 즉시 emit, z-score는 직전 7일 baseline 누적 필요)
...
- 3차 가드: z-score record 4종의 emit 확인(직전 7일 baseline 누적 부족 시 graceful skip, count > 0 시 `clamp(-5, 5)` 가드 동작 확인)
