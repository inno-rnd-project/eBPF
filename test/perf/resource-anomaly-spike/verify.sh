...
# 7일 baseline 누적이 필요 라 dev 환경 의 retention 누적 부족 시 자연 0건 으로 통과 하도록 graceful
# skip 처리한다. 본 스크립트 는 dev cluster 전용 이며 prod 에서 실행 하지 않는다.
set -euo pipefail
...
# 3차 가드 (graceful skip): z-score record 4종. 7일 baseline 누적 필요 라 dev 환경 retention 누적
...
echo "[setup] z-score record 4종 확인 (7일 baseline 부족 시 graceful skip)"
for q in "${ZSCORE_RECORDS[@]}"; do
  count=$(curl -sf --max-time 10 --data-urlencode "query=count(${q})" "${PROM_URL}/api/v1/query" \
    | python3 -c "import json,sys; r=json.load(sys.stdin)['data']['result']; print(r[0]['value'][1] if r else 0)" 2>/dev/null || echo 0)
  if [[ "${count}" == "0" ]]; then
    echo "[skip] ${q}: count=0 (7일 baseline 누적 부족, dev 환경 정상 동작)"
    continue
  fi
...
# 5차 가드: alert 4종의 severity 와 component 라벨 정합
echo "[setup] alert component=*-anomaly 라벨 정합 확인"
component_check=$(kubectl get prometheusrule -n "${NAMESPACE}" netobs-gpuobs-correlation -o json \
  | python3 -c "
import json, sys
d = json.load(sys.stdin)
expected = {
