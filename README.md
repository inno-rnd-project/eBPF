...
| `gpuobs_cuda_h2d_bytes_total` | Counter | `node`, `src_namespace`, `src_pod`, `src_pod_uid`, `gpu_uuid` | Cumulative bytes copied host→device. Sourced from cuMemcpyHtoD_v2 / cuMemcpyHtoDAsync_v2 (linear) plus cuMemcpy2D_v2 / cuMemcpy2DAsync_v2 / cuMemcpy3D_v2 / cuMemcpy3DAsync_v2 when srcMemoryType=HOST and dstMemoryType=DEVICE; total bytes for 2D/3D variants are computed as WidthInBytes × Height × Depth |
| `gpuobs_cuda_d2h_bytes_total` | Counter | `node`, `src_namespace`, `src_pod`, `src_pod_uid`, `gpu_uuid` | Cumulative bytes copied device→host. Sourced from cuMemcpyDtoH_v2 / cuMemcpyDtoHAsync_v2 plus the cuMemcpy2D_v2 / 3D_v2 family when srcMemoryType=DEVICE and dstMemoryType=HOST |
| `gpuobs_cuda_dtod_bytes_total` | Counter | `node`, `src_namespace`, `src_pod`, `src_pod_uid`, `gpu_uuid` | Cumulative bytes copied device→device. Sourced from cuMemcpyDtoD_v2 / cuMemcpyDtoDAsync_v2 plus the cuMemcpy2D_v2 / 3D_v2 family when both srcMemoryType=DEVICE and dstMemoryType=DEVICE |
| `gpuobs_cuda_unknown_dir_bytes_total` | Counter | `node`, `src_namespace`, `src_pod`, `src_pod_uid`, `gpu_uuid` | Cumulative bytes copied via memcpy paths whose direction the BPF layer cannot determine. Includes cuMemcpy (UVA, both ptrs are CUdeviceptr) and cuMemcpy2D / 3D variants whose srcMemoryType / dstMemoryType is ARRAY, UNIFIED, or HOST→HOST. Sustained nonzero rate indicates the workload uses UVA or unified memory and warrants further analysis with cuPointerGetAttribute outside this agent |
