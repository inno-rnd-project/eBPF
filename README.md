...
| `gpuobs_cuda_dtod_bytes_total` | Counter | `node`, `src_namespace`, `src_pod`, `src_pod_uid`, `gpu_uuid` | Cumulative bytes copied device→device. Sourced from cuMemcpyDtoD_v2 / cuMemcpyDtoDAsync_v2 plus the cuMemcpy2D_v2 / 3D_v2 family when both srcMemoryType=DEVICE and dstMemoryType=DEVICE. ARRAY / UNIFIED / HOST→HOST cases on the 2D/3D family are skipped at the BPF layer in this release |
