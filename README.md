...
| `gpuobs_cuda_dtod_bytes_total` | Counter | `node`, `src_namespace`, `src_pod`, `src_pod_uid`, `gpu_uuid` | Cumulative bytes copied device→device (cuMemcpyDtoD_v2 / cuMemcpyDtoDAsync_v2) captured by libcuda.so uprobes. Single-GPU intra-device copies and Driver API backend of cudaMemcpyDeviceToDevice are both covered |
