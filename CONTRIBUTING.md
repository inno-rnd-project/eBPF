...
- `internal/netobs/metadata/resolver.go`: peak 메모리 주석을 "~60MB"에서 "~200MB"로 수정
- `internal/netobs/config/config.go`: PrintEvents 기본값을 false로 변경
```
...
- 에이전트 전용 구현은 `internal/<agent>/<domain>/`에 도메인 별 패키지로 분리한다 (예: netobs는 `internal/netobs/{metadata,ebpf,config,drop,metrics,types}`, gpuobs는 `internal/gpuobs/{collector,nvml,config,metrics,types}`)
- 에이전트 무관 공용 인프라는 `internal/<domain>/`에 둔다 (예: `internal/server` HTTP 핸들러, 추후 도입될 `internal/correlation` 조인 지표)
- 패키지명은 디렉토리명과 일치시키며, 외부 라이브러리와 충돌하는 경우에만 import 시점에서 alias를 준다 (예: `ebpfx "netobs/internal/netobs/ebpf"`)
...
- 외부에서 조정이 필요한 값은 `internal/netobs/config`의 `Config` 구조체 필드로 노출하고, env와 CLI flag 양쪽에서 받을 수 있게 한다
