...
#
#   1) bump 직후 image-push-all 을 먼저 수행 후 deploy-* 실행 (가장 안전, 권장)
#   2) PR 에서 실제로 코드가 변경된 agent 만 image-push-<name> 으로 갱신하고 deploy-* 대상도 동일
#      agent 로 한정 (다른 agent overlay 도 tag 가 갱신되어 있으므로 그 agent 가 restart 되는 순간
...
bump:
	...
	echo "bumped $$CUR -> $$NEW"; \
	echo "[reminder] bump 직후 deploy-* 전에 image-push-all 또는 변경된 agent 의 image-push-<name> 을 실행하라"
