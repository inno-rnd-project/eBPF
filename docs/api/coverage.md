...
| `interference` | 자원 압박 랭킹과 노드·pod 상세, 노드 리소스 현황과 raw 사용률, 이벤트와 발화 이력, 메모리, 간섭 토폴로지 | `/api/v1/pressure`, `/api/v1/node/{node}`, `/api/v1/node/{node}/resources`, `/api/v1/pod/{namespace}/{pod}`, `/api/v1/node-vitals`, `/api/v1/events`, `/api/v1/incidents`, `/api/v1/memory`, `/api/v1/topology` |
...
| pod 단위 종합 상세 (health·vitals·cgroup CPU·네트워크) | `pod/{namespace}/{pod}` (interference). 기본 정보 (uid·node·owner) 와 pod score 환산 health 3종, limit 대비 사용률, CFS throttle 상세 (throttled 비율·periods·limit cores, cadvisor 노출 시 quota·period), pod 입도 재전송·drop rate 와 `max_srtt_seconds` (분위수 아님을 필드명으로 명시). `at` 결합 | 커버 |
| 노드 리소스 현황 (capacity·allocatable·requests·limits·usage) | `node/{node}/resources` (interference). 리소스 4종 (cpu·memory·pods·gpu) 의 kube-state 원단위 현황과 usage 와 활용률 (usage/allocatable, gpu 는 device 사용률 평균). 결측 리소스는 엔트리 생략. `at` 결합 | 커버 |
...
| GPU 일반 자원 현황 (사용률·메모리·전력·온도·상세) | `gpu-status` (gpu). `gpuobs_device_*` 와 `gpuobs_pod_*` 기반. device 상세 (SM active·클럭·팬·PCIe·performance state·온도 임계·throttle violation·encoder/decoder·bar1·energy) 와 device `idle` 판정 (rule 동일 임계 20% instant 적용), 노드별 `dominant_causes` 요약 (gpu-rca 동일 카탈로그의 cause 와 한국어 설명) 포함, node 필터 | 커버 |
