...
const (
	...
	// CudaEventDtoD 는 device→device 메모리 전송 이벤트다. emit 경로는 세 갈래다 —
	// (1) cuMemcpyDtoD_v2 / cuMemcpyDtoDAsync_v2: PARM3 ByteCount 를 그대로 누적,
	// (2) cuMemcpy2D / 3D 의 srcMemoryType=DEVICE && dstMemoryType=DEVICE 분기:
	//     WidthInBytes × Height (3D 는 × Depth) 계산값을 누적,
	// (3) libcudart cudaMemcpy / cudaMemcpyAsync 의 cudaMemcpyKind=DeviceToDevice: PARM3 count 를 그대로 누적.
	CudaEventDtoD CudaEventKind = 4
	// CudaEventUnknownDir 은 BPF 단에서 방향 판정이 불가능한 메모리 전송 이벤트다. 다음 경로가 본 kind 로 분류된다 —
	//   - UVA cuMemcpy: 양 ptr 이 모두 CUdeviceptr 라 host/device 구분 불가
	//   - cuMemcpy2D / 3D 의 ARRAY / UNIFIED / HOST→HOST srcMemoryType/dstMemoryType 조합
