...
// NewProduction은 기본 빌드에서 noop Profiler를 돌려준다. libPath 인자는 nccl_real.go의 동명
// 함수 시그니처와 정합을 위해 받되 stub에서는 사용하지 않는다. cmd/gpuobs-agent의 wire-up이
// build tag 무관하게 동일한 호출부를 유지하도록 시그니처를 일치시킨다.
func NewProduction(_ string) Profiler {
	return NewNoop()
}
