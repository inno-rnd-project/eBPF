        ...
        "/api/v1/gpu-idle": {
            "get": {
                ...
                "parameters": [
                    ...
                    {
                        "type": "string",
                        "description": "scope=node 단일 노드 필터 (DNS-1123 형식, 생략 시 전체 노드). 다른 scope 에서는 적용되지 않고 node_ignored=true 로 표기된다",
                        "name": "node",
                        "in": "query"
                    },
                    {
        ...
        "/api/v1/incidents": {
            "get": {
                ...
                "parameters": [
                    ...
                    {
                        "type": "string",
                        "description": "pod 와 함께 주면 해당 namespace 의 동명 pod 로 한정하고, 단독으로 주면 namespace/src_namespace/victim_namespace 라벨이 이 namespace 를 가리키는 에피소드만 조회",
                        "name": "namespace",
                        "in": "query"
                    }
                ],
        ...
        "/api/v1/node/{node}": {
            "get": {
                ...
                "parameters": [
                    ...
                    {
                        "type": "boolean",
                        "description": "top_pods 를 원인 후보 임계(pressure\u003e=0.4, dominant severity low 면 비움)로 필터링 (기본 false)",
                        "name": "is_filtered",
                        "in": "query"
                    }
                ],
    ...
    "definitions": {
        "internal_correlation_api.AgentHealth": {
            ...
            "properties": {
                ...
                "bpf_programs_loaded": {
                    "description": "BpfProgramsLoaded / Total 은 netobs 전용. loaded \u003c total 이면 일부 kprobe 가 detach 상태다.",
                    "type": "number"
                },
                "bpf_programs_total": {
                    "type": "number"
                ...
                "status": {
                    "description": "Status 는 healthy 또는 degraded 다. 판정 임계는 기존 알림 규칙과 동일하고, 걸린 규칙의\nalertname 이 Issues 에 담긴다.",
                    "type": "string"
                },
                "up": {
                    "type": "boolean"
        ...
        "internal_correlation_api.BandwidthResponse": {
            ...
            "properties": {
                ...
                "total": {
                    "description": "Total 은 limit 적용 전 전체 pod 수, Truncated 는 pods 가 잘렸는지다 (#352 패턴, #447 보강).\nnodes 는 클러스터 합계라 절단 대상이 아니다.",
                    "type": "integer"
                },
                "truncated": {
                    "type": "boolean"
        ...
        "internal_correlation_api.DominantPressure": {
            ...
            "properties": {
                ...
                "namespace": {
                    "description": "Namespace 와 PodName 은 pod (ns/pod 결합 id 표현, 불변) 의 분리 필드다 (#383 additive).\nnamespace 미상이면 생략된다.",
                    "type": "string"
                },
                "node": {
                    "type": "string"
        ...
        "internal_correlation_api.GpuCauseWeight": {
            ...
            "properties": {
                ...
                "chain": {
                    "description": "Chain 은 cause 의 단계형 인과 체인 문구다 (#303). 프론트가 narrative 파싱 없이 구조 필드로\n렌더할 수 있게 gpu-rca 가 채우며, gpu-idle 응답에서는 Description 과 동일하게 생략된다.",
                    "type": "string"
                },
                "description": {
        ...
        "internal_correlation_api.GpuDevice": {
            ...
            "properties": {
                ...
                "idle": {
                    "description": "Idle 은 사용률이 node:gpu_idle:5m rule 과 동일한 임계 (20% 미만) 인지의 instant 판정이다\n(#304). rule 은 5분 윈도우 비율이고 본 필드는 현재 값 판정이라 순간 부하에서는 어긋날 수 있다.",
                    "type": "boolean"
                },
                "memory_total_bytes": {
                    "type": "number"
        ...
        "internal_correlation_api.MemoryResponse": {
            ...
            "properties": {
                ...
                "total": {
                    "description": "Total 은 limit 적용 전 전체 pod 수, Truncated 는 잘렸는지다 (#352 패턴, #447 보강). 종전에는\n조용히 잘라 소비자가 절단 여부를 알 수 없었다.",
                    "type": "integer"
                },
                "truncated": {
                    "type": "boolean"
        ...
        "internal_correlation_api.NodeGpuRcaResponse": {
            ...
            "properties": {
                ...
                "gpu": {
                    "description": "Gpu 는 device 스코프 조회 시 요청 gpu 파라미터 (GPU UUID 또는 device index) 의 echo 다.",
                    "type": "string"
                },
                "idle": {
        ...
        "internal_correlation_api.NodeMapResponse": {
            ...
            "properties": {
                ...
                "page": {
                    "description": "Page 는 #411 의 opt-in 페이지네이션 additive 필드다. limit 또는 offset 을 지정한 요청에만 실리고\n미지정 요청에는 생략된다. summary 는 절단 전 전체 기준으로 산출해 요약의 의미를 보존한다.",
                    "allOf": [
                        {
                            "$ref": "#/definitions/netobs_internal_apicommon.Page"
                        }
        ...
        "internal_correlation_api.NodeResourcesResponse": {
            ...
            "properties": {
                ...
                "resources": {
                    "description": "Resources 는 리소스 종류 (cpu / memory / pods / gpu) 별 현황이다. kube-state 의 resource\n라벨을 매핑하며 (nvidia_com_gpu 는 gpu 키), 결측 리소스 (GPU 없는 노드 등) 는 엔트리가\n생략된다.",
                    "type": "object",
                    "additionalProperties": {
        ...
        "internal_correlation_api.OverviewResponse": {
            ...
            "properties": {
                ...
                "weakest": {
                    "description": "Weakest 는 health 의 가장 약한 고리 (#248) 와 동일 판정이다. health 를 아는 차원이 없으면\n생략된다.",
                    "allOf": [
                        {
                            "$ref": "#/definitions/internal_correlation_api.WeakestSignal"
                        }
        ...
        "internal_correlation_api.PodsResponse": {
            ...
            "properties": {
                ...
                "page": {
                    "description": "Page 는 #411 의 opt-in 페이지네이션 additive 필드다. limit 또는 offset 을 지정한 요청에만 실린다.",
                    "allOf": [
                        {
                            "$ref": "#/definitions/netobs_internal_apicommon.Page"
                        }
